...
def test_conversions_and_funnel(app, traffic_clock):
    with app.app_context():
        ...
        # commencé → envoyé → compte créé. The three middle steps have no data
        # here: these page views were written straight to the table, with no
        # heatmap event, no form opened and no submission behind them — which is
        # exactly what a visit from a scanner looks like.
        funnel = acquisition_funnel(days=30)
        assert [step["count"] for step in funnel] == [4, 1, 0, 0, 0, 2]
        assert funnel[2]["label"] == "Affichée dans un navigateur"
        assert funnel[3]["label"] == "Formulaire commencé"
        assert funnel[4]["label"] == "Formulaire envoyé"
